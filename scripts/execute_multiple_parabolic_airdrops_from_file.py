...
class ExecuteMultipleParabolicAirdrops:
  ...
  def __init__(self):
    ...
    self.go_to_pose.position.x = -9.0
    self.go_to_pose.position.z = 4.6
    ...
    self.filename = rospy.get_param('~configs_file', 
      str('/home/antun/catkin_ws/src/larics_motion_planning/config/airdrop_configs_fast.csv'))
        ...
        for i in range(len(self.airdrop_configs)):
          ...
          for j in range(self.drops_per_config):
            if should_publish == True:
              print " "
              print "Executing ", i+1, "/", len(self.airdrop_configs), " for ", j+1, "/", self.drops_per_config, " time"
